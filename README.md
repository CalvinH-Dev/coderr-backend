...
│   ├── permissions.py  # IsBusinessUser, IsCustomerUser
│   └── admin.py        # Admin configuration
├── offers_app/         # Service offers & packages
│   ├── models.py       # OfferPackage, Offer models
│   ├── serializers.py  # Offer package serializers
...
### Offers
```
GET    /api/offers/                 # List all offer packages (filterable)
POST   /api/offers/                 # Create offer package (business users)
GET    /api/offers/{id}/            # Retrieve offer package details
PATCH  /api/offers/{id}/            # Update offer package (owner only)
DELETE /api/offers/{id}/            # Delete offer package (admin only)
GET    /api/offers/offer/{id}/      # Get individual offer details
```

**Query Parameters for Filtering:**
- `?creator_id=123` - Filter by business user ID
- `?min_price=100` - Minimum price threshold
- `?max_delivery_time=10` - Maximum delivery time in days
- `?search=web` - Search in title/description
- `?ordering=min_price` - Order by min_price or updated_at
- `?page_size=10` - Custom page size (max 100)
...
GET    /api/orders/business/{id}/count/           # Total orders for business
GET    /api/orders/business/{id}/count-completed/ # Completed orders count
```

### Reviews
```
GET    /api/reviews/                # List all reviews
POST   /api/reviews/                # Create review (one per business user)
